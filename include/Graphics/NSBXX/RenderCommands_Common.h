...
// sizeof == 0x38
MaterialRenderData extern data_0210a278[];
...
struct RenderCommandHandler
{
    ...
    fix32_t upScale_;
    fix32_t downScale_;
    void (*callback_e8_)(BoneMatrixRenderData*, NSBXXBoneMatrix::Scaling* boneMatrixScaleData, uint8_t* ip, int boneMatrixFlags);
    void (*callback_ec_)(BoneMatrixRenderData*);
...
extern struct Struct_020f1d08
{
    ...
    char already_has_symbol_10[0x20];

    fix32_t command13Matrix4x4_[16];
} data_020f1d08;

// lookup for material bind functions, but there's only one
extern void (*data_020f1d18[])(RenderCommandHandler* handler, int modifier,  NSBXXMaterial* material, unsigned int materialIdx);
// lookup for mesh draw functions, but there's only one
extern void (*data_020f1d28[])(RenderCommandHandler*, int modifier, NSBXXMesh* mesh, unsigned int meshIdx);
